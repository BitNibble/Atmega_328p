...
// setup blank
{
	...
	exint_mask_instance()->eimsk = 0X00;
...
{
	uint8_t reset, ret = 0;
	reset = (cpu_instance()->mcusr & MCU_Control_Status_instanceister_Mask);
	switch(reset){
		case 1: // Power-On Reset Flag
			ret = 0;
			cpu_instance()->mcusr &= ~(1 << PORF);
			break;
		case 2: // External Reset Flag
			cpu_instance()->mcusr &= ~(1 << EXTRF);
			ret = 1;
			break;
		case 4: // Brown-out Reset Flag
			cpu_instance()->mcusr &= ~(1 << BORF);
			ret = 2;
			break;
		case 8: // Watchdog Reset Flag
			cpu_instance()->mcusr &= ~(1 << WDRF);
			ret = 3;
			break;
		default: // clear all status
			cpu_instance()->mcusr &= ~(MCU_Control_Status_instanceister_Mask);
			break;
	}
...
void INTERRUPT_set(uint8_t channel, uint8_t sense)
{
	switch( channel ){
		case 0: 
			exint_mask_instance()->eimsk &= ~(1 << INT0);
			exint_instance()->eicra &= ~((1 << ISC01) | (1 << ISC00));
			switch(sense){
			...
			break;
		case 1:
			exint_mask_instance()->eimsk &= ~(1 << INT1);
			exint_instance()->eicra &= ~((1 << ISC11) | (1 << ISC10));
			switch(sense){
