...
{
	timer0_state = 0;
	tc0_instance()->tccr0a &= ~((1 << WGM01) | (1 << WGM00));
	tc0_instance()->tccr0b &= ~(1 << WGM02);
	switch(wavegenmode){
		case 0: // Normal
		break;
		case 1: // PWM, Phase Correct, 8-bit
			tc0_instance()->tccr0a |= (1 << WGM00);
		break;
		case 2: // PWM, Phase Correct, 9-bit
			tc0_instance()->tccr0a |= (1 << WGM01);
		break;
		case 3: // Fast PWM
			tc0_instance()->tccr0a |= (1 << WGM00) | (1 << WGM01);
		break;
	...
	tc_imask_instance()->timsk0 &= ~((1 << OCIE0B) | (1 << OCIE0A) | (1 << TOIE0));
	switch(interrupt){
		case 0: 
		break;
		case 1:
			tc_imask_instance()->timsk0 |= (1 << TOIE0);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 2:
			tc_imask_instance()->timsk0 |= (1 << OCIE0A);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 3:
			tc_imask_instance()->timsk0 |= (1 << OCIE0B);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 4:
			tc_imask_instance()->timsk0 |= (1 << TOIE0) | (1 << OCIE0A);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 5:
			tc_imask_instance()->timsk0 |= (1 << TOIE0) | (1 << OCIE0B);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 6:
			tc_imask_instance()->timsk0 |= (1 << TOIE0) | (1 << OCIE0A) | (1 << OCIE0B);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 7:
			tc_imask_instance()->timsk0 |= (1 << OCIE0A) | (1 << OCIE0B);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		default:
		break;
...
void TIMER_COUNTER0_compareA(unsigned char compare)
{
	tc0_compare_instance()->ocr0a = compare;
}
void TIMER_COUNTER0_compareB(unsigned char compare)
{
	tc0_compare_instance()->ocr0b = compare;
}
void TIMER_COUNTER0_stop(void)
// stops timer by setting prescaler to zero
{
	tc0_instance()->tccr0b &= ~(7 << CS00); // No clock source. (Timer/Counter stopped)
	tc0_instance()->tcnt0 = 0X00;
	timer0_state = 0;
}
