...
USART0 usart0_enable(uint32_t baud, unsigned int FDbits, unsigned int Stopbits, unsigned int Parity )
{
	...
	usart0_instance()->ucsr0b = (1 << RXCIE0) | (1 << RXEN0) | (1 << TXEN0);
		...
		switch(FDbits){
			case 9:
				usart0_instance()->ucsr0b |= (1 << UCSZ02);
				usart0_instance()->ucsr0c |= (3 << UCSZ00);
			...
			case 7:
				usart0_instance()->ucsr0b &= ~(1 << UCSZ02);
				usart0_instance()->ucsr0c |= (1 << UCSZ01);
				usart0_instance()->ucsr0c &= ~(1 << UCSZ00);
		...
		switch(Stopbits){
			case 1:
				usart0_instance()->ucsr0c &= ~(1 << USBS0);
				setup_usart0.par.Stopbits = 1;
			break;
			case 2:
				usart0_instance()->ucsr0c |= (1 << USBS0);
		...
		switch(Parity){
			case 0:
				usart0_instance()->ucsr0c &= ~(3 << UPM00);
				setup_usart0.par.Parity=0;
			break;
			case 2:
				usart0_instance()->ucsr0c |= (1 << UPM01);
				usart0_instance()->ucsr0c &= ~(1 << UPM00);
...
{
	uint16_t timeout;
	usart0_instance()->ucsr0b |= 1 << UDRIE0;
	usart0_instance()->udr0 = data;
...
{ // USART, RX Complete Handler
    ...
    bit_8 = usart0_instance()->ucsr0b;
    bit_8 = 0x01 & (bit_8 >> 1);
	
	if(usr){ UART_LastRxError = usr; }
	
	UART_Rx = usart0_instance()->udr0;
