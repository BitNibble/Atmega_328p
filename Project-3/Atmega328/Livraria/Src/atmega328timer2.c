...
{
	timer2_state = 0;
	tc2_instance()->tccr2a &= ~((1 << WGM21) | (1 << WGM20));
	tc2_instance()->tccr2b &= ~(1<<WGM22);
	switch(wavegenmode){
		case 0: // Normal
		break;
		case 1: // PWM, Phase Correct
			tc2_instance()->tccr2a |=(1 << WGM20);
		break;
		case 2: // CTC
			tc2_instance()->tccr2a |= (1 << WGM21);
		break;
		case 3: // Fast PWM
			tc2_instance()->tccr2a |= (1 << WGM20) | (1 << WGM21);
		break;
	...
	tc_imask_instance()->timsk2 &= ~((1 << OCIE2B) | (1 << OCIE2A) | (1 << TOIE2));
	switch(interrupt){
		case 0: 
		break;
		case 1:
			tc_imask_instance()->timsk2 |= (1 << TOIE2);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 2:
			tc_imask_instance()->timsk2 |= (1 << OCIE2A);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 3:
			tc_imask_instance()->timsk2 |= (1 << TOIE2) | (1 << OCIE2A);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 4:
			tc_imask_instance()->timsk2 |= (1 << TOIE2) | (1 << OCIE2B);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 5:
			tc_imask_instance()->timsk2 |= (1 << OCIE2B) | (1 << OCIE2A);
			cpu_instance()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 6:
			tc_imask_instance()->timsk2 |= (1 << OCIE2B) | (1 << OCIE2A) | (1 << TOIE2);
...
//	External clock source on Tn pin. Clock on rising edge; default - clk T 0 S /1024 (From prescaler).
{
	if(timer2_state == 0){ // one shot
		tc2_compare_instance()->ocr2a = 0XFF;
		tc2_instance()->tccr2b &= ~(7 << CS20); // No clock source. (Timer/Counter stopped)
		switch(prescaler){
			case 1: // clkI/O/(No prescaler)
				tc2_instance()->tccr2b |= (1 << CS20);
			break;
			case 8: // clkI/O/8 (From prescaler)
				tc2_instance()->tccr2b |= (1 << CS21);
			break;
			case 32: // clkI/O/32 (From prescaler)
				tc2_instance()->tccr2b |= (1 << CS21) | (1 << CS20);
			break;
			case 64: // clkI/O/64 (From prescaler)
				tc2_instance()->tccr2b |= (1 << CS22);
			break;
			case 128: // clkI/O/128 (From prescaler)
				tc2_instance()->tccr2b |= (1 << CS22) | (1 << CS20);
			break;
			case 256: // clkI/O/256 (From prescaler)
				tc2_instance()->tccr2b |= (1 << CS22) | (1 << CS21);
			break;
			case 1024: // clkI/O/1024 (From prescaler)
				tc2_instance()->tccr2b |= (7 << CS20);
			break;
...
{
	tc2_instance()->tccr2a &= ~((1 << COM2A0) | (1 << COM2A1));
	switch(compoutmode){ // see table 53, 54, 55 in data sheet for more information
		...
		case 1: // Reserved
			...
			portb_instance()->ddr |= (1 << 3);
			tc2_instance()->tccr2a |= (1 << COM2A0);
		break;
...
//	default-Normal port operation, OC2 disconnected.
{
	tc2_instance()->tccr2a &= ~((1 << COM2B0) | (1 << COM2B1));
	switch(compoutmode){ // see table 53, 54, 55 in data sheet for more information
		case 0: // Normal port operation, OC2 disconnected.
		break;
		case 1: // Reserved
				// Toggle OC2 on compare match
			portd_instance()->ddr |= (1 << 3);
			tc2_instance()->tccr2a |= (1 << COM2B0);
		break;
		case 2: // Clear OC2 on compare match when up-counting.
				// Set OC2 on compare
				// match when down counting.
...
void TIMER_COUNTER2_compareA(unsigned char compare)
{
	tc2_compare_instance()->ocr2a = compare;
}
	void TIMER_COUNTER2_compareB(unsigned char compare)
{
	tc2_compare_instance()->ocr2b = compare;
}
void TIMER_COUNTER2_stop(void)
// stops timer by setting prescaler to zero
{
	tc2_instance()->tccr2b &= ~(7 << CS20); // No clock source. (Timer/Counter stopped)
	tc2_instance()->tcnt2 = 0X00;
	timer2_state = 0;
}
